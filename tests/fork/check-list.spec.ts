...
makeSuite("Mainnet Check list", (testEnv: TestEnv) => {
  ...
  it("Deposit WETH", async () => {
    const { users, wrappedTokenGateway, hWETH, pool } = testEnv;
    ...
    const hTokensBalance = await hWETH.balanceOf(user.address);

    expect(hTokensBalance).to.be.gt(zero);
    expect(hTokensBalance).to.be.gte(depositSize);
  });
  ...
  it("Withdraw WETH - Partial", async () => {
    ...
    expect(hTokensBalance).to.be.gt(zero, "User should have hTokens.");

    // Partially withdraw native ETH
    const partialWithdraw = await parseUnitsFromToken(hWETH.address, "2");

    // Approve the hTokens to Gateway so Gateway can withdraw and convert to Ether
    const approveTx = await hWETH
      .connect(user.signer)
    ...
    const afterPartialHTokensBalance = await hWETH.balanceOf(user.address);
    ...
    expect(afterPartialHTokensBalance).to.be.equal(
      hTokensBalance.sub(partialWithdraw),
      "User hWETH balance should be subtracted"
    );
  });

  it("Withdraw WETH - Full", async () => {
    const { users, hWETH, wrappedTokenGateway, pool } = testEnv;
    ...
    const afterFullHTokensBalance = await hWETH.balanceOf(user.address);
    const gasCosts = approveGas
      .mul(approveGasPrice)
      .add(withdrawGas.mul(withdrawGasPrice));

    expect(afterFullEtherBalance).to.be.eq(
      priorEthersBalance.add(hTokensBalance).sub(gasCosts),
      "User ETHER balance should contain the full withdraw"
    );
    expect(afterFullHTokensBalance).to.be.eq(
      0,
      "User hWETH balance should be zero"
    );
  });

  it("Borrow variable WETH and Full Repay with ETH", async () => {
    const { users, wrappedTokenGateway, hWETH, weth, pool, helpersContract } =
      testEnv;
  ...
  it("Borrow ETH via delegateApprove ETH and repays back", async () => {
    const { users, wrappedTokenGateway, hWETH, weth, helpersContract, pool } =
      testEnv;
    const borrowSize = parseEther("1");
    const user = users[2];
    const { variableDebtTokenAddress } =
