...
task(`upgrade-htokens`)
  ...
  .setAction(async ({ revision }, { deployments, getNamedAccounts, ...hre }) => {
    ...
    const { HTokenNamePrefix, SymbolPrefix } = await loadPoolConfig(MARKET_NAME);
    ...
    const newHtokenArtifact = await deployments.deploy(HTOKEN_IMPL_ID, {
      contract: 'HToken',
      from: deployer,
    ...
    const deployedRevision = await (
      await (await getHToken(newHtokenArtifact.address)).HTOKEN_REVISION()
    ).toString();
    if (deployedRevision !== revision) {
      console.error(
        `- Deployed HToken implementation revision ${deployedRevision} does not match expected revision ${revision}`
      );
      return false;
    }
    for (let x = 0; x < reserves.length; x++) {
      ...
      await waitForTx(
        await poolConfigurator.updateHToken({
          asset,
          treasury,
          name: `HopeLend ${HTokenNamePrefix} ${symbol}`,
          symbol: `a${SymbolPrefix}${symbol}`,
          implementation: newHtokenArtifact.address,
          params: [],
        })
      );
