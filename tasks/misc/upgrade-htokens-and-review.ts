...
task(`upgrade-htokens-and-review`)
  ...
  .setAction(async ({ revision }, { deployments, getNamedAccounts, ...hre }) => {
    const previousHTokenConfigs: { [key: string]: HTokenConfig } = await hre.run('review-htokens', {
      log: true,
    });
    ...
    const afterHTokensConfig: { [key: string]: HTokenConfig } = await hre.run('review-htokens', {
      log: true,
    });
    ...
    if (delta) {
      console.log('=== Updated HTokens, check new configuration differences ===');
