...
const func: DeployFunction = async function ({
  ...
  const hTokenArtifact = await deploy(ATOKEN_IMPL_ID, {
    contract: "HToken",
    from: deployer,
    args: [poolAddress],
  ...
  const delegationAwareHTokenArtifact = await deploy(
    DELEGATION_AWARE_ATOKEN_IMPL_ID,
    {
      contract: "DelegationAwareHToken",
      from: deployer,
      args: [poolAddress],
      ...COMMON_DEPLOY_PARAMS,
    }
  ...
  const delegationAwareHToken = (await hre.ethers.getContractAt(
    delegationAwareHTokenArtifact.abi,
    delegationAwareHTokenArtifact.address
  )) as DelegationAwareHToken;
  await waitForTx(
